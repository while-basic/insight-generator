...
Run the local Flask dashboard against an output directory:

```bash
chat-recycler dashboard --out out --host 127.0.0.1 --port 8080
```

Set `CHAT_RECYCLER_OUT` in the environment to point the dashboard at another output directory.

## Vercel deployment

The dashboard is ready to deploy with Vercel's Python runtime.

```bash
vercel deploy
```

Ensure the `CHAT_RECYCLER_OUT` environment variable points to a directory with `db.sqlite` (e.g. upload artifacts or mount a volume).
