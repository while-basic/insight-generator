...
@app.command()
def dashboard(
    output_path: Path = typer.Option("out", "--out", help="Output directory with db.sqlite"),
    host: str = typer.Option("127.0.0.1", "--host"),
    port: int = typer.Option(8080, "--port"),
    debug: bool = typer.Option(False, "--debug"),
) -> None:
    app_instance = create_app(output_path)
    log_info(f"Starting dashboard on http://{host}:{port} using {output_path}")
